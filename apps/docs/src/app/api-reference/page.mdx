...
```tsx
const overlay = useAsyncOverlay<boolean, 'close'>();
...
```tsx
const overlay = useAsyncOverlay({ restoreFocus: { selector: '#search-input' } })
```
...
```tsx
const overlay = useAsyncOverlay<{ ok: boolean }, 'close'>({
...
```tsx
function Page() {
  const overlay = useAsyncOverlay<{ ok: boolean }, 'route_change'>()
