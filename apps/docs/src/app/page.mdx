...
`use-async-overlay`는 이 둘의 절충안을 지향합니다.
...
다음은 `use-async-overlay`를 사용해 모달 2개를 순차적으로 열고 결과를 받는 데모입니다.

데모의 코드는 여기에서 확인할 수 있습니다.
...
```ts
type OverlayResolved<T> = { status: 'resolved'; value?: T }
type OverlayDismissed<R = unknown> = { status: 'dismissed'; reason?: R }
type OverlayOutcome<T, R = unknown> = OverlayResolved<T> | OverlayDismissed<R>

function useAsyncOverlay<T, R = unknown>(options?: {
  restoreFocus?:
    | boolean
    | 'previous'
    | HTMLElement
